...
private class Day22(isTest: Boolean) : Solver(isTest) {
    ...
    fun part1(): Any {
        ...
        fun Triple<Int, Int, Int>.nextStep() =
            copy(first = (first + dirs[third].first).mod(X), (second + dirs[third].second).mod(Y))
    ...
    fun part2(): Any {
        ...
        fun connectedSide(lastPos: Triple<Int, Int, Int>): Triple<Int, Int, Int> {
            ...
            return Triple(
                newpos.first + cubesize * newcube.first,
                newpos.second + cubesize * newcube.second,
                (newcube.third + 2).mod(4)
            )
        }
...
fun readAsString(name: String) = File("$name.txt").readText()
